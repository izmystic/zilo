const { EmbedBuilder, ActionRowBuilder, StringSelectMenuBuilder, ApplicationCommandOptionType, ComponentType } = require("discord.js");
...
async function search({ member, guild, channel }, query) {
  ...
  if (player && !guild.members.me.voice.channel) {
    player.vocie.disconnect();
    await guild.client.musicManager.players.destory(guild.id);
  }
  ...
  let res;
  try {
    res = await guild.client.musicManager.api.loadTracks(/^https?:\/\//.test(query) ? query : `${MUSIC.DEFAULT_SOURCE}:${query}`);
  } catch (err) {
  ...
  switch (res.loadType) {
      ...
      embed
        ...
        .addFields(
          ...
          },
          {
            ...
            value: "`" + formatTime(res.tracks.map((t) => t.info.length).reduce((a, b) => a + b, 0)) + "`",
            inline: true,
          }
        )
        .setFooter({ text: `Requested By: ${member.user.username}` });
    ...
    case "search": {
      ...
      const menuRow = new ActionRowBuilder().addComponents(new StringSelectMenuBuilder().setCustomId("search-results").setPlaceholder("Choose Search Results").setMaxValues(max).addOptions(options));

      const tempEmbed = new EmbedBuilder().setColor(EMBED_COLORS.BOT_EMBED).setAuthor({ name: "Search Results" }).setDescription(`Please select the songs you wish to add to queue`);
      ...
      try {
        ...
        if (toAdd.length === 1) {
          ...
          tracks = toAdd;
          embed.setDescription(`🎶 Added ${toAdd.length} songs to queue`).setFooter({ text: `Requested By: ${member.user.username}` });
        }
      } catch (err) {
        console.log(err);
        await sentMsg.delete();
