...
module.exports = {
  ...
  async messageRun(message, args, data) {
    ...
    // status
      ...
      if (!status || !["ON", "OFF"].includes(status)) return message.safeReply("Invalid status. Value must be `on/off`");
...
async function setChannel(settings, channel) {
  if (!channel.canSendEmbeds()) {
    return "Ugh! I cannot send greeting to that channel? I need the `Write Messages` and `Embed Links` permissions in " + channel.toString();
  }
