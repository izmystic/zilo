...
async function play({ member, guild, channel }, query) {
  ...
  try {
    const res = await guild.client.musicManager.api.loadTracks(/^https?:\/\//.test(query) ? query : `${MUSIC.DEFAULT_SOURCE}:${query}`);
    ...
    embed
      ...
      .addFields(
        ...
        },
        {
          ...
          value: "`" + formatTime(tracks.map((t) => t.info.length).reduce((a, b) => a + b, 0)) + "`",
          inline: true,
        }
      )
      .setFooter({ text: `Requested By: ${member.user.username}` });
