...
module.exports = {
  ...
  async messageRun(message, args, data) {
    ...
    if (input === "status") {
      ...
      if (!action || !["TIMEOUT", "KICK", "BAN"].includes(action)) return message.safeReply("Not a valid action. Action can be `Timeout`/`Kick`/`Ban`");
  ...
  async interactionRun(interaction, data) {
    ...
    else if (sub === "action") response = await setAction(settings, interaction.guild, interaction.options.getString("action"));
...
async function getStatus(settings, guild) {
  ...
  const logChannel = settings.modlog_channel ? guild.channels.cache.get(settings.modlog_channel).toString() : "Not Configured";
